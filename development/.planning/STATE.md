...
status: unknown
stopped_at: Completed 03.3.1-01-PLAN.md (Wave 1 fork scaffolding)
...
**Current focus:** Phase 03.3.1 — fork-education-app-into-university-erp-audit-each-doctype-fields-features-and-workflows-then-fork-into-custom-app
...
- [03.4-01 re-run 2026-04-28]: Seed script must seed Fiscal Year 2026-2027 in addition to academic year FY -- without it, GL posting fails after 2026-03-31 with "Date X is not in any active Fiscal Year"
- [03.4-01 re-run 2026-04-28]: Re-run-safe seed must include a GL backfill phase: prior runs may have left Fees in docstatus=1 with no GL entries; idempotency `continue` skips fee creation/submission, so a separate backfill is required to re-post GL entries through GLPostingManager
- [03.4-01 re-run 2026-04-28]: Backfill overrides posting_date in-memory only (no UPDATE on tabFees) so the persisted Fees record's audit trail stays intact
- [03.3.1-01]: Quiz Question table-name collision CONFIRMED (both Education and university_lms name="Quiz Question"); resolution = archive Education quiz suite under _archived/ so tabQuiz Question never registers
- [03.3.1-01]: student_sibling vs student_siblings have DISTINCT schemas (8 vs 3 fields); fork both, deprecation deferred
- [03.3.1-01]: DOCTYPE_MODULE_MAP locked at 71 entries (UNMAPPED=0); ARCHIVE_DOCTYPES=8; D-04 destination collisions=0
- [03.3.1-01]: school_house destination = University Hostel (not Student Info); relabel via JSON module rewrite
...
- Phase 03.3.1 inserted after Phase 03.3 (2026-05-14): Fork Education app into university_erp — audit each doctype, fields, features, and workflows then fork into custom app (URGENT, mirrors Accounts fork approach)
