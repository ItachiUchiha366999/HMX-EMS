...
This roadmap delivers management dashboards, analytics, and role-based portals (student, faculty, HOD, parent, management) on top of an existing Frappe v15 university ERP with 275+ doctypes. Strategy: **backend-first** — the complete Frappe Desk backend (accounts fork, workflows, data integrity, schema-linked field validation, report correctness) must be fully working before any Vue portal frontend work begins. All portal UIs will be designed and built fresh once the backend is stable, ensuring no rework.
...
### Phase 03.3.1: Fork Education app into university_erp - audit each doctype, fields, features, and workflows then fork into custom app (INSERTED)
...
- [ ] 03.3.1-02-PLAN.md — Fork academics doctypes into university_academics (archive Quiz suite + Article)
- [ ] 03.3.1-03-PLAN.md — Fork student_info + admissions doctypes; relabel School House → Hostel House
- [ ] 03.3.1-04-PLAN.md — Fork Faculty (Instructor) + Finance (Fees suite); fork 9 Education reports
- [ ] 03.3.1-05-PLAN.md — Collapse all 6 overrides into forked controllers; clean hooks.py
- [ ] 03.3.1-06-PLAN.md — Migrate Education hooks/install logic; bench migrate; verify Education uninstallable

### Phase 03.4: Post-Fork Backend Audit & Fix (INSERTED)

**Goal:** After the accounts fork, the entire university_erp backend is fully re-tested across all modules — every doctype accessible in Frappe Desk, every field linked to the correct parent schema, every report running with valid source fields, every workflow functional, all cross-app connections verified — ensuring complete backend stability before any frontend portal work begins
**Depends on:** Phase 03.3
**Requirements**: AUDIT-BACKEND-01, AUDIT-BACKEND-02, AUDIT-BACKEND-03, AUDIT-BACKEND-04, AUDIT-BACKEND-05, AUDIT-BACKEND-06
**Success Criteria** (what must be TRUE):
  ...
  7. Every Link, Dynamic Link, Table, and field reference in university_erp doctypes is validated against the exact parent doctype schema before any fix is applied
  8. Every report in university_erp is validated against the exact source doctype schema, filter fields, and join fields so reports render data correctly without field errors
**Plans**: 5 plans

Plans:
- [ ] 03.4-01-PLAN.md — Demo data wipe-and-reseed against post-3.3.1 schema per CONTEXT D-08 (April 2026 run produced 510 students/54 faculty/800 GL entries against pre-fork Education-owned doctypes; needs re-execution now that 71 doctypes have moved to university_erp modules). Idempotent script, 500+ students, 50+ faculty, full academic year per D-09.
- [ ] 03.4-02-PLAN.md — Create all missing workflows (10+) and audit all workflows E2E with role-based testing
- [ ] 03.4-03-PLAN.md — Post-fork report audit, restrict ERPNext Accounts reports, doctype accessibility audit, permission matrix re-verification
- [ ] 03.4-04-PLAN.md — Full university_erp doctype schema audit: validate every referenced field against its parent doctype and fix broken Link/Dynamic Link/Table targets
- [ ] 03.4-05-PLAN.md — Full university_erp report schema audit: validate report columns, filters, and joins against live schema and fix field-level report failures
...
| 03.4. Post-Fork Backend Audit | 1/5 | In Progress | Plan 01 re-verified 2026-04-28 |
