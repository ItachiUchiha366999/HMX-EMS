...
- [ ] **FOUND-01**: Cross-app data consistency verified (ERPNext <> HRMS <> Education <> university_erp) with documented integration points
- [ ] **FOUND-02**: SQL injection vulnerability in dashboard_engine.py fixed with parameterized queries
- [ ] **FOUND-03**: Permission checks enforced on all analytics and dashboard API endpoints (role-based access)
- [ ] **FOUND-04**: N+1 query patterns in executive_dashboard.py resolved with aggregation queries
...
- [ ] **FOUND-08**: Management role detection added (VC, Registrar, Finance Officer, Dean) beyond existing student/faculty/parent

### Shared Components

- [ ] **COMP-01**: KPI counter card component (value, label, trend indicator, status color)
- [ ] **COMP-02**: Chart wrapper component (line, bar, pie, heatmap) using ApexCharts
- [ ] **COMP-03**: Data table component with sorting, pagination, and search using TanStack Table
- [ ] **COMP-04**: Filter bar component (date range, academic year, department, program selectors)
...
- [ ] **COMP-08**: Notification/alert panel component (reusable across portals)

### Faculty Portal

- [ ] **FCTY-01**: Faculty can view today's classes with room, time, and course details
- [ ] **FCTY-02**: Faculty can mark attendance for a class with bulk-select UI (mark 60 students in under 2 minutes)
...
- [ ] **FCTY-05**: Faculty can see a pending tasks dashboard (unmarked attendance, unsubmitted grades, pending leave requests)
- [ ] **FCTY-06**: Faculty can view institutional announcements and notices
...
| FCTY-05 | Phase 3 | Pending |
| FCTY-06 | Phase 3 | Pending |
