...
from university_erp.university_finance.doctype.account.account import get_account_currency
from university_erp.university_finance.doctype.accounting_dimension.accounting_dimension import get_dimensions
...
if TYPE_CHECKING:
	# Stubbed: from erpnext.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation
	pass
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from university_erp.university_finance.general_ledger import process_debit_credit_difference
...
def remove_ref_doc_link_from_pe(
	...
	from university_erp.university_finance.doctype.payment_request.payment_request import (
...
def get_children(doctype, parent, company, is_root=False):
	from university_erp.university_finance.report.financial_statements import sort_accounts
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	# Stubbed: from erpnext.setup.setup_wizard.operations.install_fixtures import create_bank_account
...
def get_coa(doctype, parent, is_root=None, chart=None):
	from university_erp.university_finance.doctype.account.chart_of_accounts.chart_of_accounts import (
...
def repost_gle_for_stock_vouchers(
	...
	from university_erp.university_finance.general_ledger import toggle_debit_credit_if_negative
...
def _auto_create_exchange_rate_revaluation_for(frequency: str) -> None:
	...
	if companies:
		for company in companies:
			frappe.enqueue(
				"university_erp.university_finance.utils.create_err_and_its_journals",
				company=company,
				queue="long",
			)
...
def sync_auto_reconcile_config(auto_reconciliation_job_trigger: int = 15):
	...
	method = "university_erp.university_finance.doctype.process_payment_reconciliation.process_payment_reconciliation.trigger_reconciliation_for_queued_docs"
