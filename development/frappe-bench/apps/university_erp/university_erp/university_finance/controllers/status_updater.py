...
# Forked from erpnext/controllers/status_updater.py for university_finance standalone use.
# Stock/selling/buying status entries removed. Only accounting-relevant doctypes retained.
...
# Only accounting-relevant doctypes retained; stock/selling/buying entries removed.
status_map = {
	"Sales Order": [
		["Draft", None],
		[
...
class StatusUpdater(Document):
	...
	Forked for university_finance: stock-related status logic removed.
	"""
		...
		for args in self.status_updater:
			...
			for d in self.get_all_children():
				...
				if hasattr(d, "item_code") and hasattr(d, "rate") and flt(d.rate) < 0:
					frappe.throw(
						_(
							"For item {0}, rate must be a positive number."
						).format(
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role = role_allowed_to_over_bill
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		if qty_or_amount == "qty":
			action_msg = _(
				'To allow over receipt / delivery, update "Over Receipt/Delivery Allowance" in the Item.'
			)
		else:
			action_msg = _(
...
def get_allowance_for(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance is None:
			global_qty_allowance = 0  # University does not use Stock Settings
