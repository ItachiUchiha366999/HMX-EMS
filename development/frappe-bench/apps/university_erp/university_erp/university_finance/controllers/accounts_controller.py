...
# Forked from erpnext/controllers/accounts_controller.py for university_finance standalone use.
# Stock/selling/buying dependencies stripped. University-specific stubs provided.
...
from university_erp.university_finance.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimensions,
)
from university_erp.university_finance.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from university_erp.university_finance.general_ledger import get_round_off_account_and_cost_center
from university_erp.university_finance.party import (
...
from university_erp.university_finance.utils import (
...
# Stubbed: university does not use purchase rate tracking
def update_last_purchase_rate(*args, **kwargs): pass
from university_erp.university_finance.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
# Stubbed: university does not use sales/purchase returns
def validate_return(doc): pass
from university_erp.university_finance._exceptions import InvalidCurrency
# get_exchange_rate available via compat shim
get_exchange_rate = erpnext.get_exchange_rate
# Stubbed: university does not use UOM conversion from stock module
def get_uom_conv_factor(uom, stock_uom):
    return 1.0
# Stubbed: university does not use packed items
def make_packing_list(*args, **kwargs): pass
# Stubbed: university does not use stock item details
...
def get_item_tax_map(company=None, item_tax_template=None, as_json=True):
    import json as _json
    return _json.dumps({}) if as_json else {}

def get_item_warehouse(item, args, overwrite_warehouse, for_validate=False):
    return None

# Stubbed: university does not use regional overrides
from contextlib import contextmanager
@contextmanager
def temporary_flag(flag_name, value):
    old_value = getattr(frappe.local.flags, flag_name, None)
    frappe.local.flags[flag_name] = value
    try:
        yield
    finally:
        if old_value is not None:
            frappe.local.flags[flag_name] = old_value
        elif hasattr(frappe.local.flags, flag_name):
            del frappe.local.flags[flag_name]

from university_erp.university_finance.controllers.transaction_base import TransactionBase
...
class AccountsController(TransactionBase):
	...
	def on_trash(self):
		from university_erp.university_finance.utils import delete_exchange_gain_loss_journal
	...
	def calculate_taxes_and_totals(self):
		from university_erp.university_finance.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from university_erp.university_finance.party import validate_due_date
	...
	def set_missing_item_details(self, for_validate=False):
		...
		# Stubbed: university does not use serial numbers
		def get_serial_nos(serial_no): return serial_no.split("\n") if serial_no else []
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from university_erp.university_finance.utils import reconcile_against_document
	...
	def on_cancel(self):
		from university_erp.university_finance.doctype.bank_transaction.bank_transaction import (
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from university_erp.university_finance.controllers.status_updater import get_allowance_for
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from university_erp.university_finance.utils import get_company_default
...
def update_bin_on_delete(row, doctype):
	...
	# Stubbed: university does not use stock balance
	def get_indented_qty(*args, **kwargs): return 0
	def get_ordered_qty(*args, **kwargs): return 0
	def get_reserved_qty(*args, **kwargs): return 0
	def update_bin_qty(*args, **kwargs): pass
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	if parent_doctype == "Sales Order":
		# Stubbed: university does not use stock reservation
		def has_reserved_stock(*a, **k): return False
		def cancel_stock_reservation_entries(*a, **k): pass
		...
		if has_reserved_stock(parent.doctype, parent.name):
			...
			if parent.per_picked == 0:
				pass  # parent.create_stock_reservation_entries() -- stubbed
...
def merge_taxes(source_taxes, target_doc):
	from university_erp.university_finance.doctype.pos_invoice_merge_log.pos_invoice_merge_log import (
