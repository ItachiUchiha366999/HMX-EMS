...
- **Global scope**: `/workspace/development/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspace/development/.claude/notes/`)
...
2. Glob `/workspace/development/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspace/development/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspace/development/.claude/notes/` — works in any directory
